...
TextNormalizerZH::TextNormalizerZH() {

}

std::vector<std::u32string> TextNormalizerZH::split(const std::u32string &text, const std::string &lang) {
//  auto processed_text = StringToU32String(text);
  ...
  // 使用正则表达式进行分割
  static srell::u32regex SENTENCE_SPLITOR(UR"(([：、，；。？！,;?!][”’]?))");
...
std::u32string TextNormalizerZH::postReplace(const std::u32string &sentence) {
...
std::vector<std::u32string> TextNormalizerZH::Normalize(const std::u32string &text) {
...
std::u32string TextNormalizerZH::NormalizeSentence(const std::u32string &sentence) {
  auto modifiedSentence = sentence;
