...
CNBertModel::Make(TorchDevice &device, std::string_view bert_model_path, std::string_view tokenzer_path) {
  ...
  auto bert = std::make_shared<torch::jit::Module>(torch::jit::load(bert_model_path.data(), device));
  bert->eval();
